...
export class Home extends React.Component {
  ...
  componentDidMount() {
    const {
      ...
      featArticlesActivePage, getFeed, getCategories, getTopRated,
      feedActivePage, feedLimit, token, getFavoriteArticles,
      displayNextFavoriteArticle
    } = this.props;
    if (token) {
      getFeed(feedLimit, feedActivePage);
    }
    if (token) getFavoriteArticles();
    if (!token) {
      let display = 'block';
  ...
  render() {
    ...
    const {
      ...
      nextArticle, topRatedIsLoading,
      favArticleIsLoading, featArticlesIsLoading,
      feedIsLoading, categoriesIsLoading
    } = this.props;
    let displayArticles = [];
...
Home.propTypes = {
  ...
  favArticleIsLoading: PropTypes.bool.isRequired,
  topRatedIsLoading: PropTypes.bool.isRequired,
  featArticlesIsLoading: PropTypes.bool.isRequired,
...
export const mapStateToProps = (state) => {
  const {
    ...
    articlesCount: featArticlesCount, featuredArticles,
  ...
  const { categories, isLoading: categoriesIsLoading } = state.categories;
  const { topRatedArticles, isLoading: topRatedIsLoading } = state.topRated;
  const {
    favoriteArticles, nextArticle,
    isLoading: favArticleIsLoading,
